...
import { RestaBinario, RestaHexadecimal, RestaNatural } from "../restas-type";
...
export const menuResta = () => {
    ...
    rl.question("Digite su opción: ", (option: string) => {
        switch (option) {
            case '1':
                rl.close();
                recibirNumeros(new RestaNatural);
                break;
            case '2':
                rl.close();
                recibirNumeros(new RestaBinario);
                break;
            case '3':
                rl.close();
                recibirNumeros(new RestaHexadecimal);
                break;
            default:
